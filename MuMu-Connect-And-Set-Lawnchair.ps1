...
function Find-Adb {
  $candidates = @(
    ...
    "S:\Program Files\MuMu\nx_main\adb.exe",
    "S:\Program Files\MuMu\nx_device\15.0\shell\adb.exe",
    "S:\Program Files\MuMu\nx_device\12.0\shell\adb.exe",
    ".\APK.Tool.GUI\Resources\adb.exe",
    "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe"
  ) | Where-Object { $_ -and (Test-Path $_) }

  if (-not $candidates) {
    throw "找不到 adb.exe。请安装 platform-tools，或把 adb 加入 PATH。"
  }
  return $candidates[0]
}
...
function Find-MuMuVmsRoot {
  $roots = @(
    "S:\Program Files\MuMu\vms",
    "D:\Program Files\Netease\MuMu\vms",
    "C:\Program Files\Netease\MuMu\vms",
    "C:\Program Files\MuMu\vms",
    "D:\Program Files\MuMu\vms"
  )
  foreach ($r in $roots) {
    if (Test-Path $r) { return $r }
  }
  # 兜底：从运行中的 MuMuNxDevice 推路径
  $p = Get-Process MuMuNxDevice -ErrorAction SilentlyContinue | Select-Object -First 1
  if ($p -and $p.Path) {
    # ...\nx_device\15.0\shell\MuMuNxDevice.exe -> ...\MuMu\vms
    $dir = Split-Path (Split-Path (Split-Path (Split-Path $p.Path)))
    $vms = Join-Path $dir "vms"
    if (Test-Path $vms) { return $vms }
  }
  return $null
}
